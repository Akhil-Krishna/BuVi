#!/usr/bin/env bash
# Phase A3 DoD scripted flow. Needs `make up`. Idempotent: re-seeds sample-sales-db,
# resets the demo admin's MFA and removes previous demo data sources, starts the three
# services with their logs captured (so the flow can prove no secret was logged), then
# runs scripts/test_data_sources.py through api-gateway.
set -euo pipefail
...
echo "Resetting admin MFA and demo data sources"
...
for port in 8000 8001 8002; do
  if curl -sf "http://localhost:${port}/health/live" >/dev/null 2>&1; then
    echo "error: something is already serving :${port}. Stop it: this flow starts the services" \
         "itself so it can inspect their logs." >&2
    exit 1
  fi
done

LOGDIR="$(mktemp -d -t buvi-a3.XXXX)"
PIDS=()
...
GATEWAY_URL="http://localhost:8000" METADATA_DIRECT_URL="http://localhost:8002" \
  ...
  uv run --package identity-service python scripts/test_data_sources.py
