...
- **Remaining suspects:** a `SELECT … FOR UPDATE` in `_persist_usage` waiting on a lock held by the killed process's backend; a NATS or Redis await without its own bound. Neither is proven: 8 further runs instrumented with a 2-second `pg_stat_activity` poll produced 8 passes, 0 lock waits and 0 stuck transactions.

### Conclusion on the Docker Desktop theory (not open-ended)

The suspected trigger and the failure class are two different things, and only the first is environment-specific.

- **The trigger is dev/CI-only.** The flow SIGKILLs the orchestrator, which runs on the host while Postgres runs in a container, so the connection crosses Docker Desktop's userspace port forwarder. The host kernel closes the socket immediately, but the forwarder has to propagate that close into the VM, and can be slow to. Until it does, the backend sits idle in transaction holding whatever it held. In production (Section 28) services and the platform database are on one cluster network, or the database is a managed instance reached directly: a process killed on a node has its sockets closed by that node's kernel, which sends FIN/RST at once, and the backend ends its transaction immediately. **This exact topology does not exist in production.**
- **The failure class is not dev-only, and production is currently less protected than dev.** Any client that disappears *without* a FIN leaves the same state: node loss or kernel panic, a network partition, or a stateful middlebox (load balancer, pgbouncer, RDS Proxy) dropping connection state. Postgres then waits for TCP keepalives to notice. `tcp_keepalives_idle` defaults to the system value (7200 s on Linux) and `idle_in_transaction_session_timeout` defaults to `0`, so an abruptly lost worker can hold row locks for **hours**, and a resumed run would block on them exactly as suspected here.
  - Dev and CI now set `idle_in_transaction_session_timeout=60s`, `log_lock_waits=on` and `deadlock_timeout=1s` in compose.
  - Production has no equivalent: there is no Terraform yet, and platform-database sessions set no `lock_timeout` or `statement_timeout`. Only customer-facing connections (metadata connectors, query-gateway executors) set those per session.
- **Therefore** the production mitigations — server-side `idle_in_transaction_session_timeout` and explicit `tcp_keepalives_*` on the platform database, plus a `lock_timeout` on platform sessions — are required work, not an optional hardening. They are part of the Phase C1 entry requirement below, with a test that kills a lock holder and shows the waiter failing fast rather than hanging.
- **What this does not claim:** that locks caused the A5 stall. That remains unproven; the observed run showed no lock waits because the instrumentation was added afterwards. The mitigations above bound one plausible cause and are correct regardless of which cause is confirmed.

- **Status:** a **Phase C1 entry requirement** (spec d54d322), with the same weight as the real-API model test and certificate-verified data-source TLS. Phase A12 shipped detection, not a fix: a stage timeout logs the frames it was awaiting, Postgres logs lock waits with their blocker, and CI keeps every flow's service logs (ADR 0015).
