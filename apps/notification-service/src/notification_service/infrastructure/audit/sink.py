...
Webhook creation and disabling are admin actions and audited (Section 22: `webhook.*`).
`identity.audit_events` has one owner, so this service records events through identity-service's
internal API rather than writing another service's table.

Delivery retries transient failures. If it still fails, the event is logged at ERROR (without its
state) for replay and the operation stands (the same trade-off as ADR 0004).
"""
...
from notification_service.core.config import SCOPE_AUDIT_WRITE
