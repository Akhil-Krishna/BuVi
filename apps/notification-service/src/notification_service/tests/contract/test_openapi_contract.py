"""Contract: committed `contracts/openapi/notification-service.json` matches the code.
...
CONTRACT = (
    Path(__file__).resolve().parents[6] / "contracts" / "openapi" / "notification-service.json"
)
...
def test_export_matches_committed_contract() -> None:
    ...
    assert CONTRACT.is_file(), (
        "contracts/openapi/notification-service.json missing: run `make contracts`"
    )
