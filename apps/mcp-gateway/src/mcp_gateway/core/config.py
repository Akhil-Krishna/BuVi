"""mcp-gateway configuration (12-factor). Development defaults match the compose stack;
...
SCOPE_PROXY = "mcp-gateway:proxy"
SCOPE_AUDIT_WRITE = "identity-service:audit"
DENIED_SUBJECT = "mcp.invocation.denied"
...
class Settings(BaseSettings):
    ...
    service_client_id: str = "mcp-gateway"
    service_client_secret: SecretStr = SecretStr("dev-mcp-gateway-secret")
    ...
    # --- Outbound calls to MCP servers (Section 15) ------------------------------------
    ...
    #: Cap on one MCP response body (JSON or the whole SSE stream).
    max_response_bytes: int = 1_000_000
    #: Cap on a tool call's arguments, as JSON.
