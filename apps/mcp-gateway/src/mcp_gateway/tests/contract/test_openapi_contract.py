"""Contract: committed `contracts/openapi/mcp-gateway.json` matches the code.
...
CONTRACT = Path(__file__).resolve().parents[6] / "contracts" / "openapi" / "mcp-gateway.json"
...
def test_export_matches_committed_contract() -> None:
    ...
    assert CONTRACT.is_file(), "contracts/openapi/mcp-gateway.json missing: run `make contracts`"
