...
Section 22 requires "connection creation, connection secret changes" in the audit log,
and Section 7.3 requires credential changes to be audited with before/after state.
`identity.audit_events` has one owner, so this service records events through
...
(without its state) for replay and the operation stands -- a data-source credential
change is not rolled back because the audit hop was briefly down. ADR 0004 records
this trade-off and the transactional-outbox follow-up.
"""
...
from metadata_service.core.config import SCOPE_AUDIT_WRITE
