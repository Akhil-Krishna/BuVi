"""Composition root: request-scoped wiring for semantic-service."""
...
from semantic_service.application.services.definitions import DefinitionService
from semantic_service.core.config import SCOPE_PROXY, Settings
...
) -> AsyncIterator[SemanticRepository]:
...
ScopedRepo = Annotated[SemanticRepository, Depends(get_repository, scope="function")]


async def load_metric_tenant_id(metric_id: uuid.UUID, repository: ScopedRepo) -> uuid.UUID | None:
    return await repository.get_metric_tenant_id(metric_id)


def build_definition_service(request: Request, repository: SemanticRepository) -> DefinitionService:
