...
Share links expose a dashboard outside a login, so creating and revoking one is audited
(Section 7.3: always, with before/after state). `identity.audit_events` has one owner, so this
service records events through identity-service's internal API.

Delivery retries transient failures. If it still fails, the event is logged at ERROR
(without its state) for replay, and the operation stands (the same trade-off as ADR 0004).
"""
...
from dashboard_service.core.config import SCOPE_AUDIT_WRITE
