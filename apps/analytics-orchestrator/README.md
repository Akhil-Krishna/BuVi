...
| Dependencies | Postgres, Redis (event fan-out, token ledger), NATS JetStream (`ANALYTICS`, `BILLING`), identity-service, metadata-service (agent context), query-gateway (validate + execute), visualization-service (ChartSpec validation), dashboard-service (artifact store), semantic-service (approved definitions), model provider |
