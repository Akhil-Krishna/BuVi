...
* SQLAlchemy returns the pooled connection to the pool on every commit, and the next
  statement may run on a different pooled connection. A session-level setting would
  silently be missing there -- and left behind on the previous connection.
